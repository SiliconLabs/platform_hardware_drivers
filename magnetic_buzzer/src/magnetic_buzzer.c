...
sl_status_t buzzer_init (buzzer_t *buzzer)
{
  ...
  // init PWM
  retval = sl_pwm_init(&buzzer->pwm , &buzzer->config);
...
sl_status_t buzzer_end_sound(buzzer_t *buzzer)
{
  ...
  sl_pwm_set_duty_cycle(&buzzer->pwm, buzzer_VOL0);
...
sl_status_t buzzer_begin_sound(buzzer_t *buzzer, uint16_t freq)
{
  ...
  // set frequency
  sl_pwm_set_frequency(&buzzer->pwm, freq);

  // set duty cycle
  sl_pwm_set_duty_cycle(&buzzer->pwm, buzzer->volume);
