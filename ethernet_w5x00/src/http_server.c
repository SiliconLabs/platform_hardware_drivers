    ...
    case SnSR_ESTABLISHED:
      ...
      switch(socket->sock_status)
      {
        ...
        case STATE_HTTP_IDLE :
          if ((len = w5x00_socket_recv_available(socket->socknum)) > 0)
          {
            ...
            *(((uint8_t *)http->buf) + len) = '\0';
            ...
            if (socket->file_len > 0) socket->sock_status = STATE_HTTP_RES_INPROC;
            else                     socket->sock_status = STATE_HTTP_RES_DONE; // Send the 'HTTP response' end
          }
          break;
        ...
        case STATE_HTTP_RES_INPROC :
          ...
          if (socket->file_len == 0) socket->sock_status = STATE_HTTP_RES_DONE;
          break;
                                    ...
                                    const w5x00_http_server_callback_t *callback)
{
...
#ifdef W5x00_HTTP_SERVER_DEBUG
      w5x00_log_printf("> HTTPSocket[%d] : HTTP Response end - file len [ %ld ]byte\r\n",
             socket->socknum, socket->file_len);
#endif
      // Send process end
      flag_datasend_end = 1;
    }
  ...
  if (send_len) w5x00_socket_send(socket->socknum, buf, send_len);
                                 ...
                                 w5x00_http_request_t *p_http_request)
{
  ...
  switch (p_http_request->method)
  {
    ...
    case METHOD_POST :
      ...
      w5x00_log_printf("> HTTPSocket[%d] : Request uri = %s ", socket->socknum, uri_name);
      ...
      if (p_http_request->type == PTYPE_CGI)  // HTTP POST Method; CGI Process
      {
            ...
            && (file_len <= (W5x00_HTTP_SERVER_BUFFER_SIZE - (strlen(RES_CGIHEAD_OK) + 8))))
        {
          ...
          if (content_found == HTTP_RESET) http->callback->server_restart();
        } else {
          send_http_response_header(socket->socknum,
                                    http->buf,
                                    W5x00_HTTP_SERVER_BUFFER_SIZE,
 ...
 */
static void make_http_response_head(
  char *buf,   /**< pointer to response header to be made */
  uint32_t buf_size,
  char type,  /**< response type */
  uint32_t len  /**< size of response body */
  )
{
  char *head;
  ...
  len = strlen(buf);
  if (len > (buf_size - 5))  len = (buf_size - 5);
  strcat(buf, "\r\n\r\n");
  buf[len + 4] = '\0';
}
 ...
 */
static void find_http_uri_type(
 ...
 */
static void parse_http_request(
    w5x00_http_request_t *request,  /**< request to be returned */
