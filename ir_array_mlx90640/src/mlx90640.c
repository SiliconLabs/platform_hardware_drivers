...
#ifdef MLX90640_DRIVER_DEBUG_PRINT_ENABLED
#include "app_log.h"
#endif
...
static void extract_resolution_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640);
...
static void extract_Alpha_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640);
static void extract_offset_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640);
static void extract_KtaPixel_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640);
static void extract_KvPixel_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640);
...
sl_status_t mlx90640_init(sl_i2cspm_t *i2cspm_instance, uint8_t i2c_addr)
{
  ...
  if(i2c_addr != MLX90640_DEFAULT_I2C_ADDR)
  {
    status = mlx90640_set_slave_addr(i2cspm_instance, i2c_addr);
    if(status == SL_STATUS_NOT_INITIALIZED)
    {
      return SL_STATUS_NOT_INITIALIZED;
    }
  }
  status = mlx90640_I2C_Init(i2cspm_instance, i2c_addr);
  app_assert_status(status);

  status = mlx90640_dump_EE(eeMLX90640);
  ...
  bad_pixel_count = mlx90640_extract_parameters(eeMLX90640, &mlx90640);
...
sl_status_t mlx90640_get_image_array(float *pixel_array)
{
  ...
  for (uint8_t x = 0 ; x < 2 ; x++) //Read both sub-pages
  {
    ...
    int status = mlx90640_get_frame_data(mlx90640Frame);
    if (status < 0)
    {
#ifdef MLX90640_DRIVER_DEBUG_PRINT_ENABLED
    ...
    mlx90640_get_ta(mlx90640Frame, &mlx90640, &Ta);
    float tr = Ta - TA_SHIFT; //Reflected temperature based on the sensor ambient temperature
    float emissivity = MLX90640_CONFIG_EMISSIVITY;

    mlx90640_calculate_to(mlx90640Frame, &mlx90640, emissivity, tr, pixel_array);
  }
  return SL_STATUS_OK;
}
...
{
  sl_status_t status;
  status = mlx90640_I2C_change_bus_and_address(i2cspm_instance, new_addr);
  return status;
}
...
sl_status_t mlx90640_set_slave_addr(sl_i2cspm_t *i2cspm_instance, uint8_t new_addr)
{
  ...
  mlx90640_I2C_Init(i2cspm_instance, MLX90640_DEFAULT_I2C_ADDR);
  mlx90640_I2C_get_current_own_addr(&current_address);

  // If currently used i2c address is the same as the new one, then there's no need to change anything
  ...
  {
    status = mlx90640_I2C_read(0x240F, 1, &temp_address);
    if(status == SL_STATUS_TIMEOUT)
    {
      status = mlx90640_I2C_set_own_address(new_addr);
      app_assert_status(status);
      status = mlx90640_I2C_read(0x240F, 1, &temp_address);
      ...
      {
#ifdef MLX90640_DRIVER_DEBUG_PRINT_ENABLED
        app_log("\nUnknown I2C address!\nPlease find out the current address of the device, then change the MLX90640_DEFAULT_I2C_ADDR macro to that address!\n");
#endif
    ...
    else if(status == SL_STATUS_OK)
    {
      if(new_addr != 0x00)
      {
        ...
        status = mlx90640_I2C_write(0x240F, 0x0000);
        app_assert_status(status);

        // 10ms delay makes sure the write cycle finished before writing the new address
        sl_sleeptimer_delay_millisecond(10);

        // 0xBE is a reserved value in the EEPROM. 0xBExx shall be written, where xx is the i2c address
        status = mlx90640_I2C_write(0x240F, (0xBE << 8) | new_addr);
        app_assert_status(status);
 ...
 ******************************************************************************/
sl_status_t mlx90640_dump_EE(uint16_t *eeData)
{
  if(eeData == NULL)
  {
    return SL_STATUS_INVALID_PARAMETER;
  ...
  return mlx90640_I2C_read(0x2400, 832, eeData);
}
...
sl_status_t mlx90640_synch_frame(void)
{
  ...
  error = mlx90640_I2C_write(0x8000, 0x0030);
  ...
  while(dataReady == 0)
  {
    error = mlx90640_I2C_read(0x8000, 1, &statusRegister);
    if(error != 0)
    {
      return error;
    }
...
sl_status_t mlx90640_trigger_measurement(void)
{
  ...
  error = mlx90640_I2C_read(0x800D, 1, &ctrlReg);
  ...
  ctrlReg |= 0x8000;
  error = mlx90640_I2C_write(0x800D, ctrlReg);
  ...
  error = mlx90640_I2C_general_reset();
...
******************************************************************************/
sl_status_t mlx90640_get_frame_data(uint16_t *frameData)
{
  uint16_t dataReady = 0;
  ...
  error = mlx90640_I2C_read(0x0400, 768, frameData);
  if(error != 0)
  {
      return error;
  }

  error = mlx90640_I2C_read(0x0700, 64, data);
  if(error != 0)
  {
      return error;
  }

  error = mlx90640_I2C_read(0x800D, 1, &controlRegister1);
 ...
 ******************************************************************************/
sl_status_t mlx90640_extract_parameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
  int error = 0;
...
sl_status_t mlx90640_set_resolution(uint8_t resolution)
{
  ...
  if(error == 0)
  {
    ...
    error = mlx90640_I2C_write(0x800D, value);
  }
...
/***************************************************************************//**
 * Writes the desired refresh rate value in order to change the current refresh rate
 ******************************************************************************/
...
sl_status_t mlx90640_calculate_to(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, float *result)
{
  ...
  mlx90640_get_vdd(frameData, params, &vdd);
  mlx90640_get_ta(frameData, params, &ta);
...
sl_status_t mlx90640_bad_pixels_correction(uint16_t *pixels, float *to, int mode, paramsMLX90640 *params)
{
  ...
  while(pixels[pix] != 0xFFFF)
  {
    ...
    else
    {
      ...
      {
        if(is_pixel_bad(pixels[pix]-2,params) == 0 && is_pixel_bad(pixels[pix]+2,params) == 0)
        {
...
int extract_deviating_pixels(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
  ...
  else
  {
    for(pixCnt=0; pixCnt<brokenPixCnt; pixCnt++)
    {
      ...
      {
        warn = check_adjacent_pixels(mlx90640->brokenPixels[pixCnt],mlx90640->brokenPixels[i]);
        if(warn != 0)
        {
          return warn;
        }
    ...
    for(pixCnt=0; pixCnt<outlierPixCnt; pixCnt++)
    {
      ...
      {
        warn = check_adjacent_pixels(mlx90640->outlierPixels[pixCnt],mlx90640->outlierPixels[i]);
        if(warn != 0)
        {
          return warn;
        }
    ...
    for(pixCnt=0; pixCnt<brokenPixCnt; pixCnt++)
    {
      ...
      {
        warn = check_adjacent_pixels(mlx90640->brokenPixels[pixCnt],mlx90640->outlierPixels[i]);
        if(warn != 0)
        {
          return warn;
        }
